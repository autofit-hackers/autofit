    ...
    public static func applySegmentationMask(
        ...
        for _ in 0 ... (height - 1) {
            for col in 0 ... (width - 1) {
                ...
                let maskValue: CGFloat = .init(maskAddress[col])
                let backgroundRegionRatio: CGFloat = 1.0 - maskValue
                let foregroundRegionRatio = maskValue

                let originalPixelRed: CGFloat =
                    .init(imageAddress[redOffset]) / Constants.maxColorComponentValue
                let originalPixelGreen: CGFloat =
                    .init(imageAddress[greenOffset]) / Constants.maxColorComponentValue
                let originalPixelBlue: CGFloat =
                    .init(imageAddress[blueOffset]) / Constants.maxColorComponentValue
                let originalPixelAlpha: CGFloat =
                    .init(imageAddress[alphaOffset]) / Constants.maxColorComponentValue
