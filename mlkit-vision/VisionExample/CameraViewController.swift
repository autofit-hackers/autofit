...
class CameraViewController: UIViewController {
    ...
    @IBAction func selectDetector(_: Any) {
        presentDetectorsAlertController()
    }

    @IBAction func switchCamera(_: Any) {
...
//                    let outputFilename = "jointFrames" + now + ".json"
    ...
    private func resetManagedLifecycleDetectors(activeDetector: Detector) {
        if activeDetector == lastDetector {
    ...
    private func rotate(_ view: UIView, orientation: UIImage.Orientation) {
        ...
        view.transform = CGAffineTransform(rotationAngle: degree * 3.141592654 / 180)
    }
}
